...
$parentModule = 'D365FOLBDAdmin'
if (-not $ModuleName) { $ModuleName = 'D365FOLBDAdmin.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\D365FOLBDAdmin'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\D365FOLBDAdmin.psd1"
$prereqHash = @{
